...
test("should shown home page", async ({ page }) => {
  ...
  await expect(page.locator("h1")).toContainText("single-page-game-app");
  await expect(
...
test("click use this template should redirect to github generate project", async ({
  ...
  expect(newPage.url()).toBe(
    "https://github.com/sozonome/single-page-game-app/generate"
  );
});
...
test("click use this template should redirect to github repo", async ({
  ...
  await newPage.waitForLoadState();
  expect(newPage.url()).toBe("https://github.com/sozonome/single-page-game-app");
});
